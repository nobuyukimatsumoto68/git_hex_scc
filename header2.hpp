...
constexpr int nparallel = 2; // 4; //12
...
#pragma omp parallel for collapse(2) num_threads(nparallel)
  ...
  Double eps( const Idx x, const Idx y ) const { // defined on both even/odd
    assert(0<=x && x<Lx);
    assert(0<=y && y<Ly);
    assert( is_site(x,y) );
    ...
    for(int mu=0; mu<SIX; mu++){
      ...
      Idx xp, yp;
      cshift( xp, yp, x, y, mu );
      Double tmp = std::tanh(Beta[mu%3]) - (*this)(x,y)*(*this)(xp,yp);
      res += DBetaDMu[mu%3]*tmp;
    }
    res += -1.0 + div_eps;
...
#pragma omp parallel for collapse(2) num_threads(nparallel)
  ...
  Double Knew_even( int mu, const Idx x, const Idx y ) const {
    assert(0<=x && x<Lx);
    assert(0<=y && y<Ly);
    ...
    cshift( xp, yp, x, y, mu );
    const Double tmp = (*this)(x,y)*(*this)(xp,yp); // @@@
    const Double res = DBetaDKappa[mu] * ( tmp - std::tanh(Beta[mu]) ) - divs[mu];

    return res;
  }

  Double Knew_odd( int mu, const Idx x, const Idx y ) const {
    assert(0<=x && x<Lx);
    assert(0<=y && y<Ly);
  ...
  Double Knew( const int mu, const Idx x, const Idx y ) const {
    assert(0<=x && x<Lx);
    assert(0<=y && y<Ly);
    assert(0<=mu && mu<3);
    ...
    Double res = 0.0;
    if(c==0) res = Knew_even(mu,x,y);
    else if(c==2) res = Knew_odd(mu,x,y);
    else assert(false);
    return res;
  }
  ...
  Double TM_even( int mu, const Idx x, const Idx y ) const { // both even/odd
    assert(0<=x && x<Lx);
    assert(0<=y && y<Ly);
    assert( get_char(x,y)==0 );
    assert(0<=mu && mu<3);
  ...
  Double TM_odd( int mu, const Idx x, const Idx y ) const { // both even/odd
    assert(0<=x && x<Lx);
    assert(0<=y && y<Ly);
    assert( get_char(x,y)==2 );
    assert(0<=mu && mu<3);
  ...
  Double TM( const int mu, const Idx x, const Idx y ) const {
    assert(0<=x && x<Lx);
    assert(0<=y && y<Ly);
    assert(0<=mu && mu<3);
    ...
    const int c = get_char(x,y);
    Double res = 0.0;
    if(c==0) res = TM_even(mu, x,y);
    else if(c==2) res = TM_odd(mu, x,y);
    else assert(false);
    return res;
  }
